 ...
 *
 * redis 不支持 SQL 查询的 API 仅限于以下方法： where()，limit()，offset()，orderBy() 和 indexBy()。 (orderBy() 尚未实现：#1305)
 * via-关系不能通过在 redis 中没有的表定义。你只能通过其他记录来定义关系。
 * 另外，也可以定义从 redis 的活动记录关系到正常的活动记录类，反之亦然。
 *
 * 使用实例：
 *
 * 定义一个 redis 活动记录类，你的记录类需要继承自 [[yii\redis\ActiveRecord]] 并且 至少实现 attributes() 方法来定义记录的属性。
 * 一个没有指定默认值，默认为 id 的主键可以通过 [[yii\redis\ActiveRecord::primaryKey()]] 定义。
 * 主键是属性中必要的一部分，所以请确保你有一个 id 属性定义的， 如果你没有指定自己的主键。
 *
