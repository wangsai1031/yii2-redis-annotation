 ...
 * ```
 *
 * 可用命令列表和他们的参数可参阅 http://redis.io/commands。
 *
 *
...
class Connection extends Component
{
    ...
    /**
     * 测试发现这里很多方法不能用啊，特别是空格分隔的，直接执行就可以
     *
    ...
    /**
     * __call() 方法会在调用不存在的方法时被调用
     *
     * @param string $name 调用的方法名
     * @param array $params 方法的参数
     *
     *
     * 通过 __call() 方法实现：
     * 每个命令都有相应的快捷方式支持，所以可以像下面这样代替以上的命令：
    ...
    {
        // Inflector::camel2words($name, false) 将方法名由驼峰法或下划线法转换为以空格分隔的几个单词
        // strtoupper() 全部转换成大写字母
        $redisCommand = strtoupper(Inflector::camel2words($name, false));

        // 判断$redisCommand是否在命令列表中，若是就执行该方法
        if (in_array($redisCommand, $this->redisCommands)) {

            // 这里是不是写错了？？
            // 感觉应该是：
            // $this->executeCommand($redisCommand, $params);
